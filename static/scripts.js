let init = {    
        ...
        for (record of response.results) {
            ...
            for (header of headers) {
                ...
                if (header == 'diameter') {
                    ...
                    newTd.insertAdjacentHTML("afterbegin", `<button type="button" class="btn btn-secondary show-residents" data-toggle="modal" 
    ...
    addEventToButtons: function(data) {
        let buttons = document.querySelectorAll(".show-residents")
        for (button of buttons) {
            let planetName = button.dataset.id
            button.addEventListener('click', () => init.modalModifier(planetName, data))
            
        }
    },

    modalModifier: function(planetName, data) {
        let title = document.querySelector('.modal-title')
        let body = document.querySelector('.modal-body')
        title.innerHTML = "Residents of " + planetName
        let table = document.querySelector('#modalTableBody')
        table.innerHTML = ""  
        for (planet of data) {
            if (planet['name'] == planetName) {       
                for (resident of planet['residents']) {
                    fetch(resident)
                        .then((response) => response.json())
                        .then((response) => {                            
